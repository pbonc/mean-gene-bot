...
# Path to derpisms.txt in assets/txt/
...
def parse_username(arg):
    """Extracts the username, stripping @ if present."""
...
class ShoutoutCog(commands.Cog):
    ...
    async def so(self, ctx: commands.Context):
        ...
        if len(parts) != 2 or not parts[1].strip():
            await ctx.send("Usage: !so username or !so @username")
            return

        twitchname = parse_username(parts[1])
    ...
    async def os(self, ctx: commands.Context):
        parts = ctx.message.content.split(maxsplit=1)
        if len(parts) != 2 or not parts[1].strip():
            await ctx.send("Usage: !os username or !os @username")
            return
