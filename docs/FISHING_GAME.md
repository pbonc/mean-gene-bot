...
`!fish` is the only root command. Moderator/broadcaster commands `!fish on` and `!fish off` are the persistent global power switch. Power-off empties the lake and clears all active opt-ins; reopening does not automatically redeploy anyone. `!fish join` is rejected while closed. Each join starts a configured 45–90 minute outing, after which the angler leaves and must join again. Viewer commands are `join`, `stop`, `move`, `bait [species]`, `boat`, `boatcolor`, `shirt`, `sink`, `1220`, `stats`, `records`, `record`, and `diamonds`. `!fish records` shows lake records; `!fish records @user` or `!fish @user records` shows that player's biggest catch for every species they have caught. `!fish diamonds` shows the three anglers with the most Diamond catches. Species names accept aliases such as `bass`, `northern`, `eye`, `sunny`, `trout`, `catfish`, and `sturgeon`.
...
Steve can also be caught as an approximately 1-in-1,500 successful-catch event. It awards 1,000 Fishing Points and 100 lifetime Gold, increments the angler's Steve-catch stat, and prevents Steve attacks lake-wide for one hour. The Mk. 1220 rocket is an approximately 1-in-2,000 successful-catch inventory item. `!fish 1220` consumes one while the angler's boat is active, triggers the explosion effect, and awards five uniformly random species catches through the normal weight, medal, personal-best, lake-record, points, and bait-unlock rules.
